...
for i_img = 1:length(image_list)
    ...
    xmin = max(1, min(col) - crop_padding);
    xmax = min(size(view, 2) - 1, max(col) + crop_padding);
    ymin = max(1, min(row) - crop_padding);
    ymax = min(size(view, 1) - 1, max(row) + crop_padding);

    width = max(1, xmax - xmin + 1);
    height = max(1, ymax - ymin + 1);
    % fprintf('Imcrop %d %d %d %d image\n', xmin, ymin, width, height);
